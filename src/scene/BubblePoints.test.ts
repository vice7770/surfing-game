...
import { FoamField } from '../wave/FoamField';
import { ShallowWaterSolver, uniformEdges } from '../wave/ShallowWaterSolver';
import { BUBBLE_RISE_SPEED, BubblePoints } from './BubblePoints';
...
const positions = (bubbles: BubblePoints) => {
  const attribute = bubbles.mesh.geometry.getAttribute('position');
  return Array.from({ length: bubbles.mesh.geometry.drawRange.count }, (_, i) => [attribute.getX(i), attribute.getY(i), attribute.getZ(i)]);
};
...
describe('BubblePoints', () => {
  it('entrains bubbles under the breaking cell only, below the surface', () => {
    ...
    const bubbles = new BubblePoints(1);
    ...
    foam.source.fill(0);
    const quiet = new BubblePoints(1);
    quiet.update({ solver, foam }, 0.1);
    expect(quiet.mesh.geometry.drawRange.count).toBe(0);
  });
  ...
  it('rises to the surface and is gone', () => {
    ...
    expect(bubbles.mesh.geometry.drawRange.count).toBe(0);
  });

  it('replays the same bubbles for the same seed and stays within its pool', () => {
    const run = (seed: number) => {
      const { solver, foam } = breakingTank();
      const bubbles = new BubblePoints(seed, 64);
