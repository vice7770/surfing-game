...
/** A ride as the logbook keeps it: the summary, where and in what, and when (epoch ms). */
...
const BEST_KINDS: readonly BestKind[] = ['distance', 'topSpeed', 'seconds'];
...
export class Logbook {
  ...
  add(ride: LoggedRide): BestKind[] {
    ...
    const records = BEST_KINDS.filter((kind) => ride[kind] > (bests[kind] ?? -Infinity));
