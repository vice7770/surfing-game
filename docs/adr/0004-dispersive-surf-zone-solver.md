...
- Status: **Accepted (P5 and P6, 2026-09-26).** Dispersion, shoaling, refraction, group, solitary-wave, breaker-depth and Miche tests pass on the CPU reference. The CPU step (12–15 ms) misses the 4 ms budget. The WebGPU tier (P6) meets it: 3.0 ms per frame on the GPU, within tolerance of the CPU. The CPU-only fallback remains the user's decision. Decision 7 is amended: the GPU steps the worker's field and the whole field is read back each frame, with no lag, because the renderer is WebGL on the page. Decision 3's 0.5 m WebGPU cells were measured too slow (a quarter of real time) and not adopted. The Celeris benchmark comparison was replaced by analytic benchmarks. Earlier status: **Proposed (draft).** It will supersede the bulk-water parts of [ADR 0002](0002-interactive-water-field.md) once phase P2 of the [wave formation plan](../research/wave-formation-plan.md) passes validation. [ADR 0003](0003-plunging-sheet-collision.md) remains valid, amended by the mass-exchange and Iribarren-spawn rules below.
- Date: 2026-09-25
