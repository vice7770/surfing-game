...
- **Task 2 partly done** (`feat: carve on fins and rails`):
  ...
  - **Diagnosed, still open** (`it.fails` in `AttachedRider.test.ts`): after about 2.8 s at full steer, a roll oscillation throws the rider ('balance').
    ...
    - **P9 (2026-09-26): still open.** The flexible rider built only the leg along the body, because a finite leg across the body fed the same 3 Hz roll–yaw swing, so the full-lock carve still fails.
      - P9 Task 10 found the cost: the upright body cannot bank into a turn, so a full lean turns about 0.2 rad/s on a 9° rail, where Forsyth et al. 2024 measure 1.9 rad/s on a 42° rail.
      - Banking the body with the turn (by the felt force, by the path's pull, or with the board's roll) caught the rail or grew the Dutch roll, as rewrite 2 above did.
      - The next step is a rail-angle controller designed on a modal model of the board and a banked body ([P9 plan findings](2026-09-26-p9-riding.md#findings)).
