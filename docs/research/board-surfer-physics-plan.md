...
- Status: **Requirements agreed; implementation proposed**, 2026-09-25. The decisions in §0 were confirmed in a 23-question grilling session. The later Q24–Q41 decisions are specified in the [surfer P4 plan](surfer-physics-p4-plan.md), which supersedes this document's fall-body details and P2c timing. This is a companion to the [wave formation plan](wave-formation-plan.md), especially its §1.10, §3.2 and P4.
...
- Maneuvers: [surfing maneuver forces](surfing-maneuver-forces.md) maps trim, carve, pumping and future longboard nose-riding to force paths, controls and validation without treating theoretical explanations as measured coefficients.
- Integration: the [board–water handoff](board-water-integration-handoff.md) records the original P2a baseline and the later P3b/P4 checkpoint. P2c shipped view only; the worker and body sampler are P4 dependencies.
...
- Parallel preparation: the [planning backlog](parallel-planning-backlog.md) prioritizes player-flow, visual alignment, spot experience, calibration and accessibility specifications while P4 coupling is prepared.

## 0. Agreed experience and scope

The first playable target is **one configurable shortboard and rider**, with regular and goofy stance. Record the reference board's dimensions, mass, rocker and fin layout before tuning; do not silently treat one CFD board as that reference. The board is rigid in this milestone. A controlled center-of-mass rider with hand, knee and foot contacts rides it; the confirmed [surfer P4 plan](surfer-physics-p4-plan.md) adds a compact articulated body after separation. Board flex, individual fin settings and full anatomical articulation wait for a demonstrated handling gap.
...
The legacy field and new solver should each receive an adapter to a small `SurfWater` sampling interface; neither currently implements this contract. The interface returns the field at a **world position, depth and simulation step**; the caller does not know whether it came from an analytic field, a CPU grid or a GPU readback patch. Batch sampling or caller-owned output objects should avoid allocations in the worker loop.
...
At separation, preserve the rider's world pose and motion, including `ω × r` from the board, in an independent body. Keep the board active and floating; do not teleport either object. The [surfer P4 plan](surfer-physics-p4-plan.md) now specifies a compact articulated fall body, per-segment depth flow, board/lip/seabed collisions, swimming and constrained remount. The bulk water pushes the surfer without resolved body-to-grid feedback; direct lip contact exchanges impulse and the board retains its planned horizontal water reaction. No fixed three-second freeze remains. Quick retry is available during the fall. Leash forces remain a later option.

## 7. Delivery aligned with the wave plan

| Board phase | Dependency and implementation | Exit gate |
|---|---|
| **B0: seam and baseline** | At P4, once the worker and body sampler settle: add `SurfWater` adapter over legacy and new water; log current rides, turning, pop-up and fall traces. Record the reference shortboard and rider parameters. No gameplay retune yet. | Renderer, board and fall sample the same field in world coordinates; replay matches existing baseline. |
| **B1: board body** | After P4 worker/body-sampler handoff: separate board/rider mass, integrate board angular motion, buoyancy, drag and bounded planing from local-depth samples. | Static float and drop tests pass across spots; fixed-step and worker replay deterministic. |
...
| **B4: fall and calibration** | Complete P4 board consequences: orbital flow at body depth, independent rider/board aftermath, paddling and catch calibration. Add practice-wave forcing and restart around the shared solver; retain natural sets. The articulated fall, lip collision and swim/remount gates S2–S3 are detailed in the [surfer P4 plan](surfer-physics-p4-plan.md). | Wave-plan §1.10 gates plus the 30-second practice scenario and seeded natural-wave report pass; surfer-specific gates pass before recovery is called complete. |
...
This extends wave-plan P4; it does not move wave P1–P3 implementation into the board work. P2c shipped view only. B0 code waits for P4's worker and water sampler, and B1–B4 use that field to avoid implementing board physics twice. Surfer gates S0–S4 refine the fall/recovery sequence without replacing the board force plan.
...
8. **Performance and determinism.** Body/contact work must fit within the wave plan's **4 ms total CPU worker step** gate on the reference laptop. This is a target, not measured headroom: the active branch measures about **4.05 ms per 36.2k-cell solver step** in bundled Node and **5.7–7.6 ms per step** in the P2c browser main thread, before board/rider coupling. P4 needs a fresh end-to-end worker benchmark. Profile contact sampling, solver reactions and constraints separately. If the gate is missed, first reduce redundant hull patches or rider joints, then measure the resulting handling difference; preserve the shared water field, correct force directions and fall continuity. Same-build CPU replays are bit-exact; render interpolation cannot alter the physics replay.
...
- **Defer:** detailed board flex, aero forces, leash dynamics, individual finger/hand water contacts and a full anatomical ragdoll. A compact articulated fall body is in scope under the [surfer P4 plan](surfer-physics-p4-plan.md); finer anatomical detail needs a demonstrated gameplay or validation gap.
