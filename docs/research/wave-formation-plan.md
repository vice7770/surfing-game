...
|---|---:|---:|---|
| Stage 1 NLSW, CPU | ~34k | 2–4 ms | worker |
...
| **P1** | SI foundation: g = 9.81, remove `effectiveGravity` and the speed slider, time-scale, `SeaState` (JONSWAP, cos-2s spread, seeded components, Guo dispersion, analytic sampler, set predictor), physics readouts | Dispersion, beat-period and determinism tests pass. The legacy solver runs behind a flag. |
| **P2** | Spots and stage 1: composable 2D bathymetry (Beach, Point, Reef, Canyon), along-shore window, stretched grid, relaxation zone, finite-volume NLSW with wet/dry in the worker, bicubic board sampling, warm start and skip-to-set | Lake at rest over all spots; Snell refraction; Green's-law shoaling; relaxation reflection < 10 %; mass conservation; worker determinism; budget met. |
