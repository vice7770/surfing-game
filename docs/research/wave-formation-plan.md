...
- **Growth at the storm.** Fetch-limited JONSWAP: $\omega_p = 22\,(g^2/(U_{10}F))^{1/3}$ and $\alpha = 0.076\,(U_{10}^2/(Fg))^{0.22}$, capped by a fully developed Pierson–Moskowitz sea, $H_s \approx 0.21\,U_{19.5}^2/g$ ($\approx 0.22\,U_{10}^2/g$). Duration-limited growth converts duration to an equivalent fetch with USACE CEM EM 1110-2-1100 eq. II-2-35 in U₁₀, $gt/U_{10} = 77.23\,X^{0.67}$ (verified in P3b against its worked example II-2-9). The SPM 1984 form $68.8\,X^{2/3}$ uses the adjusted wind $U_A$, so it is not mixed with U₁₀. The CEM's u\*-based chain (II-2-36 to II-2-38) gives a similar Hs but a Tp about 20 % shorter and a higher fully developed cap; stage 1 keeps JONSWAP with the PM cap ($g T_p/U_{10} = 2\pi \cdot 1.026/0.877 \approx 7.35$). This reconciles passyworld's "energy ∝ wind⁴": since $H_s \propto U^2$, $E \propto H_s^2 \propto U^4$.
...
- **Travel to the spot.** Each component arrives after $t = R/c_g(\omega)$, with $c_g = g/2\omega$ in deep water. From R = 7000 km (Munk's Antarctic example), 18 s energy arrives after 5.8 days and 12 s after 8.7 days. Swell at the spot is therefore narrow-banded around $\omega = gt/2R$. The storm's size D sets the remaining bandwidth ($T_2 = T_1(R+D)/R$, Sandwell slide 18), and **distant storms give cleaner, more grouped swell**.
- **As built (P3b).** At distance R the spot sees only the waves that left the fetch F during the storm's duration D. The frequencies arriving together therefore span $\Delta\omega/\omega = (F + c_g D)/R$, and the directions span the storm's angular width F/R (a round storm). Spectral density is conserved along rays, so Hs falls by the share of the storm spectrum inside those two windows. Each window is a Gaussian with the top-hat's standard deviation (width/√12); the spectrum takes the frequency window as an optional bandwidth. Delivered spreading is capped at Goda's s_max = 75. Swell dissipation over distance is left out. For example, 18 m/s over 600 km for 36 h grows Hs 7.1 m at Tp 13.5 s. At 4000 km that delivers Hs 1.4 m, s 75 and a ±14 % band after 4.4 days. A 25 m/s storm 7000 km away delivers 2.5 m at 18 s after 5.7 days (Munk's example gives 5.8 days). The tank takes at most Hs 3 m and Tp 18 s (the buoy slider limits), and the readout says when a storm delivers more.
...
- **Wind on crests (Q23).** Wind never changes chop inside the solver, because chop is below grid resolution. It acts in two ways near crests:
  ...
  **Onset shift, sourced in P3b.** With $u = U/\sqrt{g h_b}$ (positive onshore), the breaking thresholds scale like the breaker index. They become $\gamma(1 - 0.10\,u)$ onshore and $\gamma(1 + 0.05\,|u|)$ offshore, clamped to 0.6–1.1. Lab studies find onshore wind lowers $H_b/h_b$ by up to about 40 % and offshore wind raises it by up to about 10 %, mostly by moving the breaker depth: Douglass 1990 (to $U/\sqrt{gh} = \pm 2.3$) and King & Baker 1996 (to ±1.1), reviewed by Zdyrski & Feddersen 2022. At field scale, Feddersen et al. 2023 (Surf Ranch) find a breakpoint shift of about ±10 % of surf-zone width. The overturn is larger and rounder under offshore wind: $A/H^2 \approx 0.4$ offshore against 0.2 at $U/C = 0.75$, and $W/L$ ≈ 0.48 against 0.25. The crest stress term (1) is a stage 2 item and stays qualitative.
...
$$\frac{u(z)}{\bar u}=\frac{kh\,\cosh k(z+h)}{\sinh kh}$$
  ...
  - **As built (P4b):** `PhysicalSurfWater` applies this profile with k from the peak period at the local depth, capped at 2, and keeps ū in bores (B > 0.3) and where kh < 0.05, flagging each regime; vertical flow is reconstructed from continuity. Bodies sample a Catmull-Rom surface over the render nodes (Q10) through the one `SurfWater` seam ([record](../superpowers/plans/2026-09-25-p4b-surf-water-seam.md)).
...
- **As built (G3).** The bed's light uses Maritorena's diffuse attenuation K = a + b_b, since forward-scattered light still reaches it; the beam c = a + b_p above is kept for rays through crests. Turbidity is particle scattering (beach 2, point and canyon 1, reef 0.15 m⁻¹) with an assumed particle albedo of 0.95. The crest path is the horizontal ray toward the sun, not the view ray, which rarely leaves a height-field crest's back. The water body is brightened 3× for readability. See the [record](../superpowers/plans/2026-09-25-g3-water-shading.md).
...
- **As built (G4).** Foam is split into dense whitewater (τ 3 s, inside the effective whitecap decay of 1.4–4.8 s, Callaghan et al. 2012) and a residual lace with a per-spot τ (beach 20, canyon 15, point 12, reef 8 s; surfactant-stabilised foam lasts longer, Callaghan et al. 2013). The stage 1 source is B times the bore's hydraulic-jump dissipation g q ΔH, scaled to a 0.25 m bore on 1 m. The render draws lace along Voronoi cell walls with the covered fraction equal to F, gathered into patches and baked into a tiling texture; a two-phase flow map carries it. The update costs about 0.5 ms per frame on the main thread. See the [record](../superpowers/plans/2026-09-25-g4-advected-foam.md).
...
- **Worker simulation (CPU tier).** The whole fixed-step simulation moves into a Web Worker: water, board, rider fall and lip parcels. Board and water step in lockstep and exchange reactions every step, so they belong together. **As built (P4a):** the surf zone (water, breaking, foam, lip, bubbles) runs in a module worker behind a `SurfZoneHost`, filling transferred snapshot buffers; the page draws them in 0.10 ms per frame. The worker step is 5.8 ms, above the 4 ms gate before the board is added ([record](../superpowers/plans/2026-09-25-p4a-surf-zone-worker.md)).
...
| **G2** ✓ | Far-field swell and chop, blend band, lateral surf-zone proxy. **Done** ([record](../superpowers/plans/2026-09-25-g2-far-field-ocean.md)). WKB `FarFieldProfile` tables continuous with the tank's boundary sea (Airy offshore, shoaled and γh-capped beside the window), a graded far-field mesh to ±1.5 km with Gerstner and a sky fade, and procedural shading-only chop on both water meshes. The FFT chop moves to P6. | No visible offshore seam; sets visible on the horizon; 117 tests. |
| **P3** ✓ | Emergent breaking. **P3a done** ([record](../superpowers/plans/2026-09-25-p3a-emergent-breaking.md)): a breaking indicator (Kennedy test plus a stage 1 depth-limited bore criterion, because the Kennedy rise rate is resolution dependent in the shallow-water solver), whitewater, the Iribarren breaker type, measured peel angle with close-out and skill rating, and local wind (chop and an onset shift). The surf zone needs cells of 1 m or finer. **P3b done** ([record](../superpowers/plans/2026-09-25-p3b-storm-lip-rideability.md)): storm mode (sourced duration coefficient, dispersion and angular windows), a sourced wind-on-breaking shift, a mass-conserving plunging lip (exact volume balance, once per wave, only for 0.4 ≤ ξ_b ≤ 2), and the [per-spot rideability report](rideability-report.md). At the defaults, the spots mostly close out (2–9 % of waves makeable at pro level, median peel 11–15°). The Point's straight contours refract crests parallel before they break, so peeling needs shaped spots. | Breaking-depth and Miche tests (stage 1 records its known bias); lip mass balance; per-spot rideability report generated. |
| **G3** ✓ | Fresnel, Beer–Lambert, thin-crest transmission. **Done** ([record](../superpowers/plans/2026-09-25-g3-water-shading.md)): Fresnel from n = 1.333 in three's lighting; per-spot turbidity and bed albedo; the Maritorena shallow-water reflectance (diffuse K) from a static bed texture; crest light marched toward the sun (beam c, 4 lookups); the far field shares the body model. Sandbars and the reef shelf read from above. Stage 1's broad crests let sunlight through only at their tops, so backlit glow waits for P5's peaked crests and the rendered lip. 2.9–4.2 ms GPU per physical frame on the dev Mac; there are no tiers yet (Q24). | Backlit crests show turquoise transmission and sandbars are visible from above. Budget met on all tiers. |
| **G4** ✓ | Advected foam from dissipation; bore roller; bubbles. **Done** ([record](../superpowers/plans/2026-09-25-g4-advected-foam.md)): a two-regime foam field on the solver grid (dense 3 s, per-spot lace 8–20 s), sourced by bore head-loss dissipation and lip splashes and advected semi-Lagrangian by the solver's currents; a lace network whose covered area equals the foam value, baked into a tile and carried by a two-phase flow map (about 0.1 ms GPU); bubbles under bores. The roller is the solid fresh foam on the bore front. | Foam follows dissipation and currents. Decay time is configurable per spot. |
| **P4** ✓ | Board: velocity at hull depth, paddle retune, drop and angle tests, rider fall in the orbital flow. Delivered as P4a–P4f, matching B0–B4 of the board and surfer physics plan. **P4a done** ([record](../superpowers/plans/2026-09-25-p4a-surf-zone-worker.md)): the surf zone runs in a Web Worker behind a snapshot host. **P4b done** ([record](../superpowers/plans/2026-09-25-p4b-surf-water-seam.md)): the `SurfWater` seam, the legacy board on it with bit-identical replays, and the legacy board baseline. **P4c done** ([record](../superpowers/plans/2026-09-25-p4c-board-body.md)): the rigid reference board on sampled water. Its drop down a steep face reaches 0.93 of √(v₀² + 2gΔh), with the energy ledger closed; it runs riderless in the surf zone. **P4d done** ([record](../superpowers/plans/2026-09-25-p4d-rider.md)): a separate rider with checked contacts, prone paddling near 1.6 m/s, pop-up, weight-shift steering and separation into the detached surfer, playable in the physical mode; catching waits for P4f. | Board tests (§1.10) pass. Catches work at real speeds. |
| **P5** ✓ | **Done** ([record](../superpowers/plans/2026-09-26-p5-boussinesq.md)). **Objective:** Madsen–Sørensen Boussinesq, Kennedy eddy viscosity, Tonelli–Petti backstop, ported from the Celeris reference with attribution; retire `legacy` | Dispersion error ≤ 2.5 % to kh = 3; breaking-depth tests pass without bias; benchmark cases match the Celeris reference; performance gate decided. |
| **G5–G6** ✓ | **Done** (see the ROADMAP). Caustics from real normals; spray and mist pools | Budget met per tier. |
| **P6** ✓ | **Done** ([record](../superpowers/plans/2026-09-26-p6-webgpu-tier.md)): GPU solver in the worker (3.0 ms per frame), 64 components, FFT chop. The whole field is read back each frame, with no lag, instead of an async patch; 0.5 m cells were measured too slow and not adopted. Planned: WebGPU tier: GPU solver, 0.5 m cells, 64 components, FFT chop, async patch readback | Readback lag tested. Tolerance-level replay on one device. CPU fallback unchanged. |
