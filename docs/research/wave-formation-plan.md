...
| **P2** | g = 9.81 in the solver: remove `effectiveGravity`, the Wave speed slider and the `legacy` flag. These moved here from P1 because on the legacy solver they would push the wave to about 6.3 m/s while the board is tuned for 3 m/s until P4. Spots and stage 1: composable 2D bathymetry (Beach, Point, Reef, Canyon), along-shore window, stretched grid, relaxation zone, finite-volume NLSW with wet/dry in the worker, bicubic board sampling, warm start and skip-to-set | Lake at rest over all spots; Snell refraction; Green's-law shoaling; relaxation reflection < 10 %; mass conservation; worker determinism; budget met. **P2a done** ([task plan](../superpowers/plans/2026-09-25-p2a-bathymetry-and-solver.md)). Seabeds and the stage 1 solver are validated. Lake at rest: depth change < 1e-9 m. Volume conserved to 1e-12. Stoker dam break within 5 %. Long-wave speed 1.0003 √(gh). Relaxation reflection 0.27 %. Green's law 1.3485 vs 1.3470. Snell 14.55° vs 14.41°. Run-up stays positive. P2b: sea-state boundaries, warm start, stretched grid, sliding window and performance. P2c: game integration. |
